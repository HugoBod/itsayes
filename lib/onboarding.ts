...
class OnboardingDataService implements OnboardingService {
  ...
  async completeOnboarding(pricingPlan: 'free' | 'pro' | 'team' = 'free'): Promise<{ success: boolean; workspaceId?: string; error?: string }> {
    ...
    try {
      ...
      // Set public visibility and generate public slug for free plan
      if (pricingPlan === 'free') {
        updates.is_public = true
        // The public_slug will be auto-generated by the trigger
        // But we'll handle potential duplicate slug errors in the update
      } else {
      ...
      while (updateAttempts < maxAttempts) {
        ...
        if (error.code === '23505' && error.message.includes('workspaces_public_slug_key')) {
          console.log(`⚠️ Duplicate slug error (attempt ${updateAttempts}/${maxAttempts}), retrying...`)
          
          // If free plan, try manually generating a unique slug
          if (pricingPlan === 'free' && updateAttempts < maxAttempts) {
            const currentTime = Date.now()
            const randomSuffix = Math.random().toString(36).substring(2, 8)
            updates.public_slug = `wedding-project-${currentTime}-${randomSuffix}`
            console.log(`🔄 Trying with manual slug: ${updates.public_slug}`)
            continue
          }
        }
