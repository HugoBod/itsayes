...
export default function WeddingStylePage() {
  ...
  return (
    <OnboardingLayout
      ...
      description="By understanding your vision — from colors to mood — I'll craft inspiration boards, vendor suggestions, and planning steps that align perfectly with your dream."
      onBack={handleBack}
      onNext={handleNext}
    ...
    >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {THEME_OPTIONS.map((theme) => (
            <div key={theme.value} className="flex items-center space-x-2">
              ...
              />
              <label htmlFor={theme.value} className="text-sm text-gray-700 cursor-pointer flex items-center gap-2">
                <Icon name={theme.icon as keyof typeof import('@/components/ui/icons').iconRegistry} className="h-4 w-4 text-gray-500" />
                {theme.label}
              </label>
            </div>
          ))}
        ...
        <div className="flex items-start space-x-2 mt-3">
          <Checkbox
            id="other-theme"
            checked={selectedThemes.includes('other')}
            onCheckedChange={(checked: boolean) => {
              handleThemeChange('other', checked)
              if (!checked) setOtherTheme('')
            }}
          />
          <div className="flex-1">
            <label htmlFor="other-theme" className="text-sm text-gray-700 cursor-pointer block mb-2">
              Other
            </label>
            {selectedThemes.includes('other') && (
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:border-primary focus:outline-none"
              />
            )}
          </div>
        </div>
      ...
      <div>
            ...
            >
              ...
              {selectedColorPalette === palette.name && (
                <div className="flex justify-center mt-1">
                  <Icon name="checkCircle" className="h-3 w-3 text-primary" />
                </div>
              )}
            </div>
          ))}
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.value && !COLOR_PALETTE.some(p => p.name === e.target.value)) {
              setSelectedColorPalette('')
            }
          }}
          placeholder="Or describe your own palette: e.g., Blush, Gold, White"
