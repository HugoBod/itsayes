...
// Import as ES modules using dynamic import since the files are TypeScript
import('./lib/moodboard-randomizer.js').then(async ({ buildScene }) => {
const { WEDDING_CATEGORIES, GLOBAL_ELEMENT_POOLS } = await import('./lib/moodboard-categories.js')
const { generateBatchPrompts } = await import('./lib/moodboard-prompts.js')
...
try {
  ...
  result.photos.forEach((photo, index) => {
    const elementCount = Object.keys(photo.elements).filter(key => 
      !key.endsWith('Subtle') && photo.elements[key]
    ).length
